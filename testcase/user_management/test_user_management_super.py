...
class TestUserManagementSuper:
    ...
    def test_table(self, user_page):
        try:
            ...
            assert user_page.get_page_title() == '高级用户列表'
            with allure.step('表格筛选学校'):
                user_page.filter_school('东莞理工学院')
                user_page.filter_user_kind()
                time.sleep(0.1)
                arr = user_page.get_all_school_in_page()
                assert len(arr) == 1 and '东莞理工学院' in arr
            with allure.step('表格筛选用户类型'):
                user_page.filter_school()
                user_page.filter_user_kind('教师')
                time.sleep(0.1)
                arr = user_page.get_all_user_kind_in_page()
                assert len(arr) == 1 and '教师' in arr
            with allure.step('同时筛选'):
                user_page.filter_school('东莞理工学院')
                user_page.filter_user_kind('教师')
                time.sleep(0.1)
                arr = user_page.get_all_school_in_page()
    ...
    @pytest.mark.parallel
    @allure.title('筛选学校')
    def test_filter_school(self, user_page, db_conn):
        try:
            allure.dynamic.description(f'前置条件：\n1. 超管登录后点击教材管理-用户管理')
            with allure.step('数据库插入新学校'):
                new_school = '新学校'
                with db_conn.cursor() as cursor:
                    cursor.execute(f"INSERT INTO t_school (SCHOOL_NAME) "
                                   f"VALUES ('{new_school}')")
                    db_conn.commit()
            with allure.step('查看筛选学校'):
                user_page.driver.refresh()
                user_page.top_side_bar.switch_to_user_management()
                assert new_school in user_page.get_all_school_in_menu()
            with allure.step('删除刚插入的新学校'):
                with db_conn.cursor() as cursor:
                    cursor.execute(f"DELETE FROM t_school WHERE SCHOOL_NAME = '{new_school}'")
                    db_conn.commit()
            with allure.step('查看筛选学校'):
                user_page.driver.refresh()
                user_page.top_side_bar.switch_to_user_management()
                assert new_school not in user_page.get_all_school_in_menu()
        except Exception as e:
            screenshot(user_page.driver)
            raise e
        finally:
    ...
    def test_update_detail(self, user_page, db_conn, datas):
        try:
            ...
            with allure.step('点击详情'):
                user_page.driver.refresh()
                user_page.top_side_bar.switch_to_user_management()
                user_page.filter_school(school)
